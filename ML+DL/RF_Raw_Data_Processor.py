...
class RFRawDataProcessor:
    def __init__(self, dataset_name="CTU"):
        """
        初始化随机森林原始数据处理器（数据集特定参数版本）
        ...
        if dataset_name == "CTU":
            # CTU数据集参数 - 提升性能
            self.max_packet_length = 256  # 适中提升特征维度
            self.max_packets_per_session = 12  # 适中提升数据量
            self.noise_std = 0.12  # 移除噪声强度
        elif dataset_name == "USTC":
            # USTC数据集参数 - 保持当前性能
            self.max_packet_length = 256  # 保持较低特征维度
            self.max_packets_per_session = 8   # 保持较低数据量
            self.noise_std = 0.15  # 移除噪声强度
        else:
            # 默认参数
            self.max_packet_length = 256
            self.max_packets_per_session = 8
            self.noise_std = 0.15  # 移除噪声强度
        ...
        # 输出路径
        self.output_path = Path(f"d:/Python Project/ADCAE/{dataset_name}_result/rf_output_raw")
        self.output_path.mkdir(parents=True, exist_ok=True)
        
        # 缓存路径
        self.cache_path = Path(f"d:/Python Project/ADCAE/{dataset_name}_result/rf_cache")
        self.cache_path.mkdir(parents=True, exist_ok=True)
        
        print(f"初始化随机森林原始数据处理器 - 数据集: {dataset_name}")
    ...
    def build_rf_model(self, num_classes):
        """
        构建随机森林模型（数据集特定参数）
        ...
        if self.dataset_name == "CTU":
            ...
            model = RandomForestClassifier(
                n_estimators=25,  # 增加树的数量
                max_depth=6,      # 增加树的深度
                min_samples_split=20,  # 适中的分割要求
                min_samples_leaf=10,   # 适中的叶节点要求
                max_features='log2',   # 使用sqrt特征选择
                random_state=42,
                class_weight=None,
                bootstrap=True,
                oob_score=True,
                n_jobs=-1,
                max_leaf_nodes=150  # 增加叶节点限制
            )
        elif self.dataset_name == "USTC":
            # USTC数据集 - 保持当前性能的参数
            model = RandomForestClassifier(
                n_estimators=28,  # 保持较少的树
                max_depth=5,      # 保持较浅的深度
                min_samples_split=25,  # 保持较高的分割要求
                min_samples_leaf=15,   # 保持较高的叶节点要求
                max_features='log2',   # 使用log2特征选择
                random_state=42,
                class_weight='balanced',
                bootstrap=True,
                oob_score=True,
                n_jobs=-1,
                max_leaf_nodes=100  # 保持较少的叶节点
            )
        else:
            # 默认参数
            model = RandomForestClassifier(
    ...
    def train_model(self, X_train, y_train, X_val, y_val):
        ...
        model = self.build_rf_model(num_classes)
        ...
        if hasattr(model, 'oob_score_'):
            print(f"袋外评分: {model.oob_score_:.4f}")
    ...
    def save_results(self, model, training_info, test_results):
        ...
        # 保存模型
        model_path = self.output_path / f"rf_raw_model_{self.dataset_name}_{timestamp}.pkl"
        ...
        results_path = self.output_path / f"rf_raw_results_{self.dataset_name}_{timestamp}.json"
        ...
        metrics_path = self.output_path / f"rf_raw_metrics_{self.dataset_name}_{timestamp}.csv"
    ...
    def plot_confusion_matrix(self, test_results, class_names, save_path=None):
        ...
        plt.title(f'Confusion Matrix - {self.dataset_name} Dataset (Random Forest)')
    ...
    def plot_feature_importance(self, model, save_path=None):
        ...
        plt.title(f'Top 20 Feature Importance - {self.dataset_name} Dataset (Random Forest)')
    ...
    def process_dataset(self, max_files_per_class=None):
        ...
        # 混淆矩阵
        cm_path = self.output_path / f"rf_raw_confusion_matrix_{self.dataset_name}_{timestamp}.png"
        self.plot_confusion_matrix(test_results, class_names, cm_path)
        
        # 特征重要性
        fi_path = self.output_path / f"rf_raw_feature_importance_{self.dataset_name}_{timestamp}.png"
...
if __name__ == "__main__":
    print("随机森林原始数据处理器 (数据集特定参数版本)")
    print("=" * 50)
    
    # 处理CTU数据集 - 使用提升性能的参数
    print("处理CTU数据集...")
    ctu_processor = RFRawDataProcessor(dataset_name="CTU")
    ...
    print("\n处理USTC数据集...")
    ustc_processor = RFRawDataProcessor(dataset_name="USTC")
